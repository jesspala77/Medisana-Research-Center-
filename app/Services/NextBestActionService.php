...
class NextBestActionService
{
    public function determine(Lead $lead): string
    {
        ...
        return match ($lead->industry?->slug) {
            ...
            'bond-agency-insurance' => $this->bondAction($lead),
            'cnc-machining-services' => $this->cncAction($lead),
            default => 'Review lead',
        };
    }
    ...
    private function cncAction(Lead $lead): string
    {
        $profile = $lead->cncQuoteProfile;
        if (! $profile) {
            return 'Complete CNC quote profile';
        }
        if (! $profile->material || ! $profile->quantity) {
            return 'Request missing RFQ details';
        }

        return 'Review RFQ details and confirm manufacturability';
    }
}
